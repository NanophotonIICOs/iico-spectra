...
from PyQt5.QtCore import QRunnable, QObject, pyqtSignal, Qt, QThreadPool,QThread
...
class Worker(QRunnable):
    ...
    def run(self):
        ...
        while self.is_running:
            ...
            self.signals.measurementFinished.emit([wavelengths, intensities])
...
class SpectrometerApp(QMainWindow):
    ...
    def start_measurement(self):
        ...
        self.data = []
        self.wavelengths = []
        self.worker = Worker(self.spectrometer, self.integration_time)
        self.worker.signals.measurementFinished.connect(self.process_measurement)

        # Start the worker thread using the thread pool
    ...
    def stop_measurement(self):
        ...
        # Show measurement finished alert
        if self.save_file_radio.isChecked():
            ...
            self.show_alert("Measurement finished. Data not saved.")
            self.data_saved = False
    ...
    def save_data(self):
        if self.data and self.save_file_radio.isChecked():
            ...
            file_name = f"{self.file_name}-{self.integration_time:.1f}ms-{date_str}.csv"
            file_path = save_file_with_number(file_name, self.file_path)
            try:
    ...
    #         self.stop_measurement()  # Detener el hilo antes de salir
    ...
    def exit_application(self):
        ...
        if confirm_exit == QMessageBox.Yes:
            if self.is_measuring:
                ...
                alert.setText("Measurement in progress. Do you want to stop and exit?")
...
def main():
    ...
    spectrometer_app.threadpool = QThreadPool()  # Crear el QThreadPool
    sys.exit(app.exec_())
